...
local scr_title = 'aa_insert_FX_on_touch_time_adjustment_loser.lua'
...
-- parse line
  ...
  function main(tr)
	...
	local fxId = TrackFX_AddByName( tr, "time_adjustment_loser", false, 0 )
	--reaper.ShowConsoleMsg( fxId )
	if fxId < 0 then
		fxId = TrackFX_AddByName( tr, "time_adjustment_loser", false, -1 )
        --SetFXName( tr, fxId, "JS: Time Adjustment Delay" )
        SNM_AddTCPFXParm( tr, fxId, 1 )
	end 

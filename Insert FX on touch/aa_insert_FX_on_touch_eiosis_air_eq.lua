...
local scr_title = 'aa_insert_FX_on_touch_eiosis_air_eq.lua'
...
-- parse line
  ...
  function main(tr)
	...
	local fxId = TrackFX_AddByName( tr, "VST2:Eiosis AirEQ Premium (Eiosis)", false, 0 )
	--reaper.ShowConsoleMsg( fxId )
	if fxId < 0 then
		fxId = TrackFX_AddByName( tr, "VST2:Eiosis AirEQ Premium (Eiosis)", false, -1 )
		_ = reaper.TrackFX_SetParam( tr, fxId, 20, 0.37 ) -- Band 1 Freq 516Hz
		_ = reaper.TrackFX_SetParam( tr, fxId, 61, 0.62 ) -- Band 5 Freq 2.6kHz
		_ = reaper.TrackFX_SetParam( tr, fxId, 6, 100 ) -- Band Earth ON
		_ = reaper.TrackFX_SetParam( tr, fxId, 16, 100 ) -- Band 1 ON
		_ = reaper.TrackFX_SetParam( tr, fxId, 57, 100 ) -- Band 5 ON
		_ = reaper.TrackFX_SetParam( tr, fxId, 74, 100 ) -- Band Air ON
